...
class Main
  ...
  def play_game(master_mind_service, display_service, turn, play_again, answer, feedback_summary)
    if play_again
      ...
      feedback_sum = master_mind_service.provide_user_summary(feedback_summary, turn, guess, feedback)
      ...
      if master_mind_service.check_for_win(feedback)
        ...
        play_game(master_mind_service, display_service, 1, master_mind_service.play_again(play_again),
                  master_mind_service.generate_code, feedback_sum) unless !play_again
      elsif turn == LOSE
        display_service.lose_message
        play_again = gets.chomp.upcase
        play_game(master_mind_service, display_service, 1,
                  master_mind_service.play_again(play_again), master_mind_service.generate_code, feedback_sum) unless !play_again
      else
        play_game(master_mind_service, display_service, turn + 1, true, answer, feedback_sum)
      end
    end
  end
end
